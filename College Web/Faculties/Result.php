...
#$sql = "SELECT *FROM `Results` WHERE SPU"
        if(isset($_POST['Search']) && $Count == 1){
        $sql = "SELECT *FROM `Student Registration` WHERE SPU_ID ='$Spuid'";
        $check = mysqli_query($conn, $sql);
        while($Match = $check->fetch_assoc()){
    ...
    <?php if(isset($_POST['WholeClassResultDeclared'])){
        ...
        echo "<script>window.location=' SelectExam.php'; </script>";
    } ?>
        </center>
        <p>
        <center>
       ...
       if($_SERVER['REQUEST_METHOD'] and isset($_POST['Result'])){
        ...
        $sql = "INSERT INTO `Results`(SPU_ID, Name, Class, S1, S2, S3, S4, S1M, S2M, S3M, S4M, PassingMarks, Total, Obtained) VALUES('$Spuid', '$Name', '$Class', '$S1', '$S2', '$S3', '$S4', '$S1M', '$S2M', '$S3M', '$S4M', $PassingMarks, '$Total', '$Obtained')";
        ...
        if($Class == 'BCA-5' or $Class == 'BCA-6'){
            $sql = "INSERT INTO `Results`(SPU_ID, Name, Class, S1, S2, S3, S1M, S2M, S3M, PassingMarks, Total, Obtained) VALUES('$Spuid', '$Name', '$Class', '$S1', '$S2', '$S3', '$S1M', '$S2M', '$S3M', $PassingMarks, '$Total', '$Obtained')";
            if(mysqli_query($conn, $sql)){
                echo "<h4 style='color: white; background: green;'>SPUID : <span style='color: skyblue; font-size: 19.9px;'>$Spuid</span>`s result is declared.</h4>";
            }
           }
           else{
            $sql = "INSERT INTO `Results`(SPU_ID, Name, Class, S1, S2, S3, S4, S5, S6, S7, S8, S1M, S2M, S3M, S4M, S5M, S6M, S7M, S8M, PassingMarks, Total, Obtained) VALUES('$Spuid', '$Name', '$Class', '$S1', '$S2', '$S3', '$S4', '$S5', '$S6', '$S7', '$S8', '$S1M', '$S2M', '$S3M','$S4M', '$S5M', '$S6M','$S7M', '$S8M', $PassingMarks, '$Total', '$Obtained')";
            if(mysqli_query($conn, $sql)){
                echo "<h4 style='color: white; background: green;'>SPUID : <span style='color: skyblue; font-size: 19.9px;'>$Spuid</span>`s result is declared.</h4>";
            }
           }
        ...
        while($Match = $check->fetch_assoc()){
            ...
            Sign in with your login ID in the college website to check.";
